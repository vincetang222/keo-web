...
export default {
  async fetch(request, env, ctx) {
    ...
    // POST /consent — log bằng chứng đồng ý cookie (Nghị định 13 + Luật 91/2025/QH15 yêu cầu
    // chứng minh được sự tuân thủ). KHÔNG lưu IP hay bất kỳ định danh cá nhân nào — chỉ lưu lựa
    // chọn (analytics/marketing) + thời điểm + phiên bản chính sách, gắn với 1 mã ngẫu nhiên trả
    // về cho trình duyệt tự giữ (để chính user có thể tra cứu lại bằng chứng đồng ý của họ sau này
    // nếu cần, không phải để Kèo truy vết ai đã đồng ý gì).
    // LƯU Ý TRIỂN KHAI: cần tạo KV namespace tên CONSENT_KV trong Cloudflare dashboard rồi bind
    // vào wrangler.toml (`wrangler kv:namespace create CONSENT_KV`), route này sẽ lỗi 500 tới khi
    // đó vì env.CONSENT_KV chưa tồn tại.
    if (request.method === 'POST' && pathname === '/consent') {
      try {
        ...
        await env.CONSENT_KV.put(`consent:${id}`, JSON.stringify(record), { expirationTtl: 60*60*24*365*2 }); // giữ 2 năm
        return new Response(JSON.stringify({ ok: true, consentId: id }), { headers: corsHeaders });
      } catch (err) {
        return new Response(JSON.stringify({ error: 'Không log được lựa chọn cookie: ' + err.message }), { status: 500, headers: corsHeaders });
      }
    }
    ...
    try {
      ...
      else return new Response(JSON.stringify({ error: 'Route không tồn tại. Dùng /football/top, /basketball/nba, /volleyball, /weather/vn-cities, hoặc POST /consent' }), { status: 404, headers: corsHeaders });
    } catch (err) {
